...
They can see the "Mamatha" referral, which is currently marked as **"REFERRED"**. They click the **"Check-in"** button to confirm the patient's arrival.


### Step 4: The Referral is Completed

After check-in, the patient's status is updated to **"CHECKED_IN"**. The referral is now considered complete, and the automated alert for this patient is cancelled.
...
1.  **If the patient ("Mamatha") does *not* check in** within the 72-hour window specified in Step 2...
2.  A backend automated job (`escalationWorker.js`) finds all referrals that are still "REFERRED" and past their urgency deadline.
